    ...
    <aside x-cloak x-show="sidebarOpen"
        ...
        <div class="p-3" style="border-top: 1px solid #e2e8f0;">
            <div class="flex items-center gap-2.5 px-2 py-2 rounded-lg hover:bg-slate-50 transition-colors">
                ...
                <a href="{{ route('profile.edit') }}" class="p-1 rounded text-slate-400 hover:text-slate-600 hover:bg-slate-100 transition-colors" title="Pengaturan Profil">
            ...
            <!-- Tombol Logout — selalu tampil, termasuk di mobile -->
            <form method="POST" action="{{ route('logout') }}" class="mt-1">
                @csrf
                <button type="submit"
                    class="w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm font-medium text-red-600 hover:bg-red-50 transition-colors">
        ...
        <header class="topbar sticky top-0 z-30">
            <div class="flex items-center h-14 px-4 lg:px-6 gap-3">
                ...
                <div class="ml-auto flex items-center gap-2">
                    ...
                    {{-- Mobile: tombol "Keluar" teks kecil di topbar, selalu terlihat --}}
                    <form method="POST" action="{{ route('logout') }}" class="lg:hidden">
                        @csrf
                        <button type="submit"
                            class="flex items-center gap-1 px-2.5 py-1.5 rounded-md text-xs font-semibold text-red-600 hover:bg-red-50 border border-red-200 transition-colors">
